"""Elo Production Pipeline: Two-Stage Model with Elo as PRODUCTION model.

Uses the original log-based MOV multiplier Elo formula (K=20, carryover=0.75,
margin_factor=True, home_advantage=0) from src/features.py.

Steps:
1. Build features with Elo for both genders
2. Train two-stage models (Stage 1 + Stage 2, Optuna 50 trials each)
3. Evaluate on 2025 holdout to confirm target Brier scores
4. Retrain production models on ALL data
5. Save production models and metadata
6. Regenerate submission files
"""
...
# Elo parameters -- fixed (original log-based MOV formula defaults)
ELO_K = 20
ELO_CARRYOVER = 0.75
ELO_MARGIN = True
...
# ===================================================================
# STEP 1: Compute Elo Ratings (fixed params: K=20, carryover=0.75, margin=True)
# ===================================================================
print("\n" + "=" * 70)
...
print(f"{'='*60}")
print(f"  Men's Brier:    {m_2s_scores['brier']:.4f}  (target ~0.1498)")
print(f"  Women's Brier:  {w_2s_scores['brier']:.4f}  (target ~0.1139)")
print(f"  Combined Brier: {c_2s_scores['brier']:.4f}  (target ~0.1318)")
print(f"  Men's Accuracy: {m_2s_scores['accuracy']:.3f}")
print(f"  Women's Accuracy: {w_2s_scores['accuracy']:.3f}")

# ===================================================================
# STEP 12: Production Retrain on ALL Data
# ===================================================================
print("\n" + "=" * 70)
print("STEP 12: Production retrain on ALL data (2003-2025 M / 2010-2025 W)")
print("=" * 70)
...
# No calibration (previous run showed it hurts)
...
# ===================================================================
# STEP 13: Generate Submission Files (VECTORIZED)
# ===================================================================
print("\n" + "=" * 70, flush=True)
print("STEP 13: Generating submission files (vectorized)", flush=True)
print("=" * 70, flush=True)
...
def generate_submission_vectorized(
    ...
    # Pre-compute lookups
    seed_lookup = {}
    for df, gender_flag in [(m_seeds, True), (w_seeds, False)]:
...
# ===================================================================
print("\n\n" + "=" * 60)
print("=== PRODUCTION MODEL SAVED (with Elo) ===")
print(f"Men's 2025 Brier: {m_2s_scores['brier']:.4f}")
print(f"Women's 2025 Brier: {w_2s_scores['brier']:.4f}")
print(f"Combined 2025 Brier: {c_2s_scores['brier']:.4f}")
print(f"Models: {OUTPUT_DIR / 'm_two_stage_final.joblib'}, {OUTPUT_DIR / 'w_two_stage_final.joblib'}")
print(f"Submissions: {OUTPUT_DIR / 'submission_stage1.csv'}, {OUTPUT_DIR / 'submission_stage2.csv'}")
print("Website data: pending (run prepare_data.py next)")
print("=" * 60)
